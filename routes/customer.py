...
from services.customer_service import navigate_to_customer_daily_schedule, download_excel_for_customer_date
from services.customer_parser import parse_customer_excel
...
async def get_customers(
    dates: List[str] = Query(
        ..., description="조회할 고객 날짜 리스트 (예: ?dates=2025-12-10&dates=2025-12-15)")
):
    driver = None
    try:
        clean_dirs()
        ...
        for curr in target_dates:
            ...
            f = download_excel_for_customer_date(driver, curr)
            if f:
                data = parse_customer_excel(f, d_str)
